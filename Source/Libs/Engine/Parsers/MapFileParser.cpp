...
auto MapFileParser::Parse() -> std::expected<bool, std::string> {
  ...
  std::map<SceneSelect, nlohmann::json> vJsonFiles;
  ...
  auto append_new_json_file = [&vJsonFiles](std::pair<SceneSelect, std::string> mapFiles) -> void {
    Logger::Debug(
      LogType::Log,
  ...
  auto each_json_file = [&](const std::pair<SceneSelect, nlohmann::json>& mapFile) {
