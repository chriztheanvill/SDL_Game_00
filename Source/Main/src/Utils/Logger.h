...
class Logger {
  public:
	...
	static auto
	Debug(LogType log = LogType::Log, std::string_view message = "", Ts&&... params) -> void {
	...
	// template <typename... Ts>
	// static void WriteOnFile(Ts... text)
	// {
	...
	// static void CanWrite(const bool& can = false) { write = can; }
	// static void ANewFile( )
	// {
	// 	std::ofstream newFile(GetFileName( ));
	// 	newFile.close( );
	// }
	...
	static void WriteOnFile(T t) {
		// if (!write) { return; }
		std::ofstream myfile(GetFileName( ), std::fstream::app);
		myfile << t;
		myfile.close( );
	}
