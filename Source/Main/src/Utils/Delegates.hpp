...
template<typename... Ts> constexpr bool is_pack_empty = false;

// Specialization for the case when the parameter pack is empty
template<> constexpr bool is_pack_empty<> = true;

/**
 * @brief Action(C#) estilo C++
 * @params:
...
/**
 * @brief Func(C#) estilo C++
 * @params:
 * R = Tipo de retorno
 * Sum = Sumar el resultado de todas las funciones, O, solo regresar el ultimo.
 * Ts = Varios parametros
 */
template<class R, bool Sum = false, typename... Ts> class Func {
 public:
  // Concept to ensure the callable is compatible
  template<typename Func>
    requires std::invocable<Func, Ts&&...>
