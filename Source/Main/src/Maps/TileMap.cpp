...
auto TileMap::SelectLevel(uint16_t level) -> void {
  ...
  assert(m_registry->HasSystem<RenderSystem>() || !fprintf(stderr, "RenderSystem not found: \n"));
...
auto TileMap::ClearOtherLevels(uint16_t level) -> void {
  ...
  for (Entity& entity: m_registry->GetSystem<RenderSystem>().GetEntities()
                         | views::filter([&](Entity& entity) -> bool {
    ...
    return tile == nullptr || tile->level != level || !entity.HasComponent<GraphicComponent>();
  })) {
    ...
    for (const auto& [name, tiles]: vecs) {
      ...
      for (const Tile& eachTile: tiles) {
        ...
        if (!eachTile.frames.empty()) {
          ...
          // Logger::Debug(LogType::Debug, "TileMap::AddTileMapToRegistry No animations");
