...
class Pool : public IPool {
  ...
  public:
	...
	auto Get(uint16_t id) -> T& {
		...
		// 	counter++;
		// }
		// return static_cast<T&>(m_data.at(newID));

		auto findit = find_if(m_data, [id](const T& val) { return val.entityID == id; });
		// // if (findit == m_data.end( )) {
		// // 	Logger::Debug(LogType::Log, "---- Pool::id ", id);
		// // 	// Logger::Debug(LogType::Log, "---- Pool::id ", static_cast<T&>(*findit));
		// // 	// return std::unexpected("Pool::Get(id) Not found id");
		// // }
