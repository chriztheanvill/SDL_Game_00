...
private:
  ...
  template <class T>
  auto FindSystems( ) const -> std::expected<
...
template <typename T>
auto Registry::FindSystems( ) const -> std::expected<
  std::unordered_map<std::type_index, std::shared_ptr<System>>::const_iterator,
  std::string> {
...
auto Registry::GetSystem( ) const -> std::shared_ptr<T> {
  ...
  assert(systemfound || !fprintf(stderr, "Error: %s\n", systemfound.error( ).c_str( )));
...
auto Registry::RemoveComponent(Entity& entity) -> void {
  ...
  // NOTE: ESTE assert no lo he probado, puede fallar
  assert(
	m_componentPools.contains(componentID)
	|| !fprintf(
	  stderr,
	  "ESTE assert no lo he probado, puede fallar -- No component found: %s\n",
