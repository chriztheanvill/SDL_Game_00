...
void GameStateManager::SetTextureManagerRenderer(SDL_Renderer* render)
{
...
void GameStateManager::Update(const float& deltaTime)
{
	...
	{
		mController.Events( );
		if (mController.Quit( ) || !mController.GetIsRunning( ))
		{
			Logger::Debug(LogType::Error, "GameStateManager::Update Quit!!!");
			mIsRunning = false;
			return;
		}
		...
		std::unique_ptr<EngineState> tmp = mCurrent->Update(deltaTime);
		Render( );
		if (tmp)
		{
...
// --------------------------------------------------------------------------------------
void GameStateManager::SetState(std::unique_ptr<EngineState>& state)
{
